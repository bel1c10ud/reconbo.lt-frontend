export const i18nMessage = {
  ...
  IF_REGION_INCORRECT: {
    "en-US":
      "If your region settings are incorrect, you will not be able to get the correct information.",
  ...
  LEGAL_NOTICE: {
    ...
    "ko-KR":
      "이 프로젝트는 Riot Games가 보증하지 않으며 Riot Games 또는 Riot Games 속성을 생산하거나 관리하는 데 공식적으로 관련된 사람의 견해나 의견을 반영하지 않습니다. Riot Games 및 모든 관련 자산은 Riot Games, Inc.의 상표 또는 등록 상표입니다.",
    "ja-JP":
      "このプロジェクトは、Riot Games によって承認されておらず、Riot Games または Riot Games プロパティの作成または管理に公式に関与している人物の見解や意見を反映するものではありません。 Riot Games および関連するすべてのプロパティは、Riot Games, Inc. の商標または登録商標です。",
  },
  INTRODUCTION_RECONBOLT: {
    "en-US":
      "reconbo.lt is a web service that allows you to check your valorant store without running the game client.",
    "ko-KR":
      "reconbo.lt는 게임 클라이언트를 실행하지 않고 발로란트 상점을 확인할 수 있는 웹서비스입니다.",
    "ja-JP":
      "reconbo.ltはゲームクライアントを実行せずにバローラントショップを確認できるウェブサービスです。",
  },
  INTRODUCTION_NOTICE: {
    "en-US":
      "In this project, we are implementing it using the previous login API, and in some functions, we are using Headless Browser and the rewrite function of Next.js to enable integrated operation in the web browser environment.",
    "ko-KR":
      "이 프로젝트에서는 비공식 Login API를 구현하여 사용하고 있으며, 일부 기능에서는 웹브라우저 환경에서 정상적으로 동작할 수 있도록 Headless Browser와 Next.js의 rewrite 기능을 사용하고 있습니다.",
    "ja-JP":
      "このプロジェクトでは非公式ログインAPIを実装して使用しており、一部の機能ではウェブブラウザ環境で正常に動作できるようにHeadless BrowserとNext.jsのrewrite機能を使用しています。",
  },
