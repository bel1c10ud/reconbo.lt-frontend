  ...
  const type = useMemo(() => {
    if(router.query.type) {
      if(typeof router.query.type === 'string') {
        ...
        if(Object.keys(ExternalAPI.Endpoint).find(endpoint => endpoint === type[type.length-1]))
        return router.query.type[router.query.type.length-1] as keyof typeof ExternalAPI.Endpoint;
      }
    }
