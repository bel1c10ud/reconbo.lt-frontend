...
export default function BundleDetail() {
  ...
  const clientAPIBundle = useMemo(() => {
    ...
    if (clientAPIStore.error) return { ...obj, error: clientAPIStore.error };
      ...
      if (
        clientAPIStore.data.FeaturedBundle.Bundles.find(
          (bundle: any) => bundle.DataAssetID === uuid,
        )
      )
        return {
          ...obj,
          data: clientAPIStore.data.FeaturedBundle.Bundles.find(
            (bundle: any) => bundle.DataAssetID === uuid,
          ),
        };
      else return { ...obj, error: new Error("not found client api bundle data") };
    }
  ...
  const externalAPIBundle = useMemo(() => {
    ...
    if (externalAPIBundles.error) return { ...obj, error: externalAPIBundles.error };
      ...
      else return { ...obj, error: new Error("not found external api bundle data") };
    }
  ...
  else if (externalAPIBundle.isLoading || !externalAPIBundle.data)
    return <BundleDetailLayoutSkeleton />;
  else {
    return (
      <BundleDetailLayout
        auth={auth}
