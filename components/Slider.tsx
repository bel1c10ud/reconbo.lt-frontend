...
import { useState, useRef, useEffect, useCallback, SyntheticEvent, useMemo } from "react";
    ...
    if (props.auto) {
      const cb = () => {
        ...
        if (pageEl) {
          ...
          const positions = Array.from(pageEl.children).map(
            (el) => el.getBoundingClientRect().left - startX,
          );
          const currentIndex = positions
            .map((x) => Math.abs(currentX - x))
          ...
          if (currentIndex + 1 < length) {
            pageEl.scrollTo({ left: positions[currentIndex + 1], behavior: "smooth" });
          } else {
            pageEl.scrollTo({ left: positions[0], behavior: "smooth" });
          }
        }
      };
...
export function ScrollBar(props: { contentRef: RefObject<HTMLDivElement> }) {
  ...
  return (
    <div className={style["scroll-bar"]}>
      <div className={style["scroll-bar-track"]} ref={scrollTrackRef} onClick={trackClickHandler}>
        <div className={style["scroll-bar-track-line"]}></div>
      </div>
      <div
        className={style["scroll-bar-thumb"]}
        ref={scrollThumbRef}
        style={{
          width: `${thumbWidth}%`,
          left: `${thumbLeft}%`,
