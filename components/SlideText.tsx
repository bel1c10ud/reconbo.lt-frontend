...
import style from "@/components/SlideText.module.css";
  ...
  const slideSize = useMemo(() => {
    if (wrapEl.current?.offsetWidth && textEl.current?.offsetWidth) {
      if (wrapEl.current.offsetWidth >= textEl.current.offsetWidth) {
        ...
        return (
          wrapEl.current.offsetWidth *
          Math.floor(textEl.current.offsetWidth / wrapEl.current.offsetWidth)
        );
      }
    } else {
      return undefined;
  ...
  useEffect(() => {
    ...
    if (textEl.current && wrapEl.current) {
      if (slideSize) {
        ...
        textEl.current.style.animationDuration =
          (6000 / wrapEl.current.offsetWidth) * slideSize + "ms";
