...
import Slider, { ScrollBar } from "@/components/Slider";
import { usePlayValPageData } from "@/hooks";
import type { AsyncData, ValorantOfficialWeb } from "@/type";
import style from "@/components/LatestNews.module.css";
...
export default function LatestNews() {
  ...
  const selectedArticlesData = useMemo(() => {
    const result: AsyncData<ValorantOfficialWeb.Article[]> = {
      isLoading: false,
      error: undefined,
      data: undefined,
    };
    ...
    else if (pageData.data && pageData.data.data.contentstackHomepage.news.articles_select)
      return { ...result, data: pageData.data.data.contentstackHomepage.news.articles_select };
  ...
  const articlesData = useMemo(() => {
    ...
    else if (pageData.data && pageData.data.data.allContentstackArticles.nodes)
      return { ...result, data: pageData.data.data.allContentstackArticles.nodes };
  ...
  const newsData = useMemo(() => {
    ...
    else if (selectedArticlesData.error || selectedArticlesData.data?.length === 0) {
      //선택된 Article이 없거나 오류가 발생했을 떄
  ...
  }, [articlesData, selectedArticlesData]);

  if (newsData.isLoading) return <LatestNewsSkeleton />;
  else if (newsData.error || !newsData.data)
    return <LatestNewsError error={newsData.error ?? new Error("not found newsData!")} />;
  return (
...
function LatestNewsArticle(props: { data: ValorantOfficialWeb.Article }) {
  const pageData = usePlayValPageData();
  ...
  }, [pageData.data, props.data.article_type, props.data.external_link, props.data.url?.url]);
