  ...
  return (
    <>
      ...
      <div className={style.self}>
        <div className={style.headline}>{i18nMessage["LOGIN"][language]}</div>
        <form
          className={!(process === "MANUAL") ? style.hidden : undefined}
          ...
          <Callout>
            ...
            <CalloutBody>
              <p className={style.description}>
                {i18nMessage["PASTE_THE_REDIRECTED_ADDRESS_INTO_THE_URL_FIELD"][language]}
              </p>
              <Link
        ...
        <form
          className={!(process === "AUTH") ? style.hidden : undefined}
          onSubmit={(e) => e.preventDefault()}
        >
          <Callout>
            <CalloutTitle>ℹ️ {i18nMessage["PLEASE_SELECT_REGION"][language]}</CalloutTitle>
            <CalloutBody>{i18nMessage["IF_REGION_INCORRECT"][language]}</CalloutBody>
          </Callout>
          <RegionSelect disabled={isProcess} />
          <TextInput
            type="text"
        ...
        <form
          className={!(process === "MULTI") ? style.hidden : undefined}
          onSubmit={(e) => e.preventDefault()}
        >
          <Callout>
            <CalloutTitle>
              ℹ️ {i18nMessage["PLEASE_ENTER_VERIFICATION_CODE"][language]}
            </CalloutTitle>
          </Callout>
          <Input
            type="text"
            name="code"
            placeholder="Multifactor Code"
            value={code}
            onChange={(e) => setCode(e.target.value)}
          />
          <button
            className={style["submit-button"]}
            onClick={onClickMulti}
